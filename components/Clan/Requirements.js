    ...
    <Card noPad>
      ...
      <View style={{flexDirection:"row"}}>
        <ScrollView horizontal={true} style={{flex:1}} contentContainerStyle={{flexDirection:"row",minWidth:'100%',paddingLeft:55}}>
          ...
          <View style={{flexDirection:"column",flexGrow:1,alignItems:"stretch",backgroundColor:level_colors.ind}}>
            ...
            <View style={{flexDirection:"row"}}>
              {(data?.data?.order?.individual??[]).map(i=><View key={`Individual${i}`} style={{flexGrow:1}}>
                ...
                {data?.data?.levels?.map(l=><View key={l.id} style={{marginHorizontal:-1,height:24,padding:4,alignItems:"center",backgroundColor:level_colors[l.id]}}>
          ...
          {reward&&<View style={{flexDirection:"column",flexGrow:1,alignItems:"stretch",backgroundColor:level_colors.ind}}>
            <View style={{height:24,padding:4}}><Text style={{textAlign:"center",fontWeight:"bold",fontSize:12}}>Rewards</Text></View>
            <View style={{flexDirection:"row"}}>
              {(data?.data?.order?.rewards??[]).map(i=><View key={`Reward${i}`} style={{flexGrow:1}}>
                <View style={{height:60,borderBottomWidth:2,borderBottomColor:level_colors.border,padding:4,alignItems:"center"}}>
                  ...
                  <Text numberOfLines={1} style={{textAlign:"center",fontWeight:"bold",fontSize:10}}>{data?.data?.rewards?.[i]?.short_name??data?.data?.rewards?.[i]?.name.replace(/Virtual /,'V').replace(/Physical /,'P').replace(/Flat /,'').replace(/ Mystery/,'').replace(/THE /,'').replace(/ Wheel/,'W').replace(/Hammock/,'Hamm')}</Text>
                  {/* <Text numberOfLines={1} style={{textAlign:"center",fontSize:12}}>{data?.data?.rewards?.[i]?.name.split(' ').slice(1).join(' ')}</Text> */}
                  {/* <Text numberOfLines={1} style={{textAlign:"center",fontSize:12}}>{data?.data?.requirements?.[i]?.bottom}</Text> */}
                </View>
                {data?.data?.levels?.map(l=><View key={l.id} style={{marginHorizontal:-1,height:24,padding:4,alignItems:"center",backgroundColor:level_colors[l.id]}}>
                  <Text style={{textAlign:"center",width:'100%',fontSize:12}}>{l.rewards?.[i]?.toLocaleString()}</Text>
                </View>)}
              </View>)}
            </View>
          </View>}
        ...
        <View style={{width:56,position:"absolute",left:0,top:0,borderRightWidth:2,borderRightColor:level_colors.border}}>
          ...
          <View style={{height:reward?60:77,backgroundColor:level_colors.null,flexDirection:"row",alignItems:"center",padding:4,borderBottomWidth:2,borderBottomColor:level_colors.border}}><Text numberOfLines={1} ellipsizeMode="head" style={{fontSize:12}}>Levels</Text></View>
